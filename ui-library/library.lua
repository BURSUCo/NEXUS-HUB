...
function Library.new(config)
	...
	function WindowTable:NewTab(cfg)
		...
		-- ============================================================
		-- 🔥 LINORIA-STYLE SECTION SYSTEM (înlocuiește NewSection)
		-- ============================================================
		-- Folosești AddLeftGroupbox('Titlu') / AddRightGroupbox('Titlu')
		-- în loc de NewSection({Position="Left", Title="Titlu"})
		-- ============================================================

		local function CreateGroupbox(title, position)
			-- Creează un groupbox simplu și curat (ca LinoriaLib)
			-- Păstrează componentele existente (NewToggle, NewButton, etc.)
			...
			local UICorner_Block = Instance.new("UICorner")
			local GroupboxAutoUI = Instance.new("UIListLayout")

			Groupbox.Name = "Groupbox"
			Groupbox.Parent = (position == "Left" and LeftFrame) or RightFrame
			...
			Groupbox.Size = UDim2.new(0.980000019, 0, 0, 200)
			Groupbox.ClipsDescendants = true
			Twen:Create(Groupbox, TweenInfo1, {BackgroundTransparency = 0.75}):Play()

			UICorner.CornerRadius = UDim.new(0, 3)
			UICorner.Parent = Groupbox

			UIStroke.Transparency = 1
			UIStroke.Color = Color3.fromRGB(255, 255, 255)
			UIStroke.Parent = Groupbox
			Twen:Create(UIStroke, TweenInfo1, {Transparency = 0.9}):Play()

			-- Titlul groupbox-ului (simplu, fără iconiță)
			...
			TitleLabel.Text = title or "Groupbox"
			TitleLabel.TextColor3 = Color3.fromRGB(200, 200, 200)
			TitleLabel.TextSize = 13
			TitleLabel.TextTransparency = 1
			TitleLabel.TextWrapped = true
			TitleLabel.TextXAlignment = Enum.TextXAlignment.Left
			Twen:Create(TitleLabel, TweenInfo2, {TextTransparency = 0.15}):Play()

			-- Linie separatoare sub titlu
			BlockFrame.Name = "BlockFrame"
			BlockFrame.Parent = Groupbox
			...
			BlockFrame.Position = UDim2.new(0, 5, 0, 24)
			BlockFrame.Size = UDim2.new(1, -10, 0, 1)
			BlockFrame.ZIndex = 3
			Twen:Create(BlockFrame, TweenInfo2, {BackgroundTransparency = 0.8}):Play()

			UICorner_Block.CornerRadius = UDim.new(0.5, 0)
			UICorner_Block.Parent = BlockFrame

			-- Auto-layout pentru componente
			GroupboxAutoUI.Name = "GroupboxAutoUI"
			GroupboxAutoUI.Parent = Groupbox
			GroupboxAutoUI.HorizontalAlignment = Enum.HorizontalAlignment.Center
			GroupboxAutoUI.SortOrder = Enum.SortOrder.LayoutOrder
			GroupboxAutoUI.Padding = UDim.new(0, 3)

			GroupboxAutoUI:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
				Twen:Create(Groupbox, TweenInfo.new(0.1), {
					Size = UDim2.new(0.98, 0, 0, math.max(GroupboxAutoUI.AbsoluteContentSize.Y + 30, 50))
				}):Play()
			end)

			-- ==================================================================
			-- COMPONENTELE RĂMÂN EXACT ACELEAȘI (NewToggle, NewButton, etc.)
			-- ==================================================================

			function GroupboxTable:NewToggle(toggle)
				...
				FunctionToggle.Parent = Groupbox
			...
			function GroupboxTable:NewTitle(lrm)
			...
			function GroupboxTable:NewButton(cfg)
				cfg = Config(cfg,{
					Title = "Button",
					Callback = function() end;
				});
				...
				FunctionButton.Parent = Groupbox
			...
			function GroupboxTable:NewKeybind(ctfx)
				ctfx = Config(ctfx,{
					Title = "Keybind",
					Callback = function() end,
				...
				local BindEvent = Instance.new('BindableEvent',Groupbox);
				...
				FunctionKeybind.Parent = Groupbox
			...
			function GroupboxTable:NewSlider(slider)
				...
				FunctionSlider.Parent = Groupbox
			...
			function GroupboxTable:NewDropdown(drop)
				drop = Config(drop,{
					Title = "Dropdown",
				...
				FunctionDropdown.Parent = Groupbox
			...
			function GroupboxTable:NewTextbox(conf)
				conf = Config(conf,{
					Title = "Textbox",
					Default = '',
					FileType = "",
				...
				FunctionTextbox.Parent = Groupbox
		...
		-- 🔥 Noile funcții: AddLeftGroupbox / AddRightGroupbox
		-- (înlocuiesc complet pe NewSection)
		function TabTable:AddLeftGroupbox(title)
			return CreateGroupbox(title, "Left")
		end

		function TabTable:AddRightGroupbox(title)
			return CreateGroupbox(title, "Right")
		end
...
print('[ OK ]: Fetch Nothing Library (with Linoria-style sections)')
